...
serve(async (req) => {
  ...
  try {
    console.log("tv-auth-complete: request received", req.method)
    ...
    console.log("tv-auth-complete: anonHeader present:", !!anonHeader, "authHeader present:", !!authHeader, "expectedAnon present:", !!expectedAnon)
    console.log("tv-auth-complete: anonHeader length:", anonHeader?.length, "expectedAnon length:", expectedAnon?.length)
    ...
    console.log("tv-auth-complete: hasValidApiKey:", hasValidApiKey, "hasValidBearer:", hasValidBearer)

    if (!hasValidApiKey && !hasValidBearer) {
      console.log("tv-auth-complete: UNAUTHORIZED - key mismatch")
    ...
    if (intent === "signup") {
      console.log("tv-auth-complete: attempting signup for", email)
      ...
      console.log("tv-auth-complete: signup response status:", signupResp.status)
      if (!signupResp.ok) {
        const txt = await signupResp.text().catch(() => "")
        console.log("tv-auth-complete: signup FAILED:", txt)
        const message = txt.trim() ? `Unable to create account (${txt})` : "Unable to create account"
        return new Response(JSON.stringify({ error: message }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
      console.log("tv-auth-complete: signup succeeded")
    }

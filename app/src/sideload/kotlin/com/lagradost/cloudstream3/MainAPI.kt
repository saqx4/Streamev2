...
abstract class MainAPI {
     ...
     * If a plugin overrides this one, Streame dispatches to it via reflection
 ...
 **/
// Rhino JS engine not bundled in Streame — plugins that call getRhinoContext() will fail gracefully.
suspend fun getRhinoContext(): Any {
    throw UnsupportedOperationException("Rhino not available in Streame")
}
    ...
    override fun getTotalEpisodeIndex(episode: Int, season: Int): Int {
        ...
        return this.episodes.maxOf { (_, episodes) ->
            episodes.count { episodeData ->
                // Prioritize display season as season may be something random to fit multiple seasons into one.
                val episodeSeason =

...
import com.streame.tv.ui.components.catalogueRowLayoutKeyFromPreferenceName
import com.streame.tv.ui.components.catalogueRowLayoutPreferencePrefixFor
import com.streame.tv.ui.components.normalizeCardLayoutMode
import com.streame.tv.ui.components.profileCatalogueRowLayoutModeKey
    ...
    private suspend fun applyCloudPayload(payload: String) {
        ...
        root.optJSONArray("hiddenPreinstalledCatalogs")?.toString()?.let { json ->
            if (!root.has("hiddenPreinstalledByProfile")) {
                val hidden: List<String> = if (json.isBlank()) {
                    emptyList()
                } else {
        ...
        // show ever partially watched in Streame (written to cloud by
